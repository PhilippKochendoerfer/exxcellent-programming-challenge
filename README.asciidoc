...
Then to run the main class _de.exxcellent.challenge.App_::
    `mvn compile exec:java "-Dexec.args=--weather path/to/weather.csv"` +
    `mvn compile exec:java "-Dexec.args=--football path/to/football.csv"`

Or without the exec plugin::
    `mvn compile` +
    `java -cp target/classes de.exxcellent.challenge.App --weather path/to/weather.csv`
