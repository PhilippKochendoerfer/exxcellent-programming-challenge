...
Then to run the main class _de.exxcellent.challenge.App_::
    `mvn compile exec:java "-Dexec.args=--weather --csv --file path/to/weather.csv"` +
    `mvn compile exec:java "-Dexec.args=--football --json --file path/to/football.json"` +
    `mvn compile exec:java "-Dexec.args=--weather --json --url https://example.org/weather.json"`

Or without the exec plugin::
    `mvn compile` +
    `java -cp target/classes de.exxcellent.challenge.App --weather --csv --file path/to/weather.csv`

Supported data formats::
    Both CSV and JSON input are supported via the `--csv`/`--json` format
    flag. A JSON source is expected to contain an array of flat objects,
    one per row, e.g. `[{"Day":"1","MxT":"88",...}, ...]`.

Supported data sources::
    Data can be read from a local file or a URL via the `--file`/`--url`
    source flag, independent of the format flag above.
